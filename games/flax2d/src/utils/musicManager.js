export class musicManager {
    ...
    setMusic(track) {
        switch (track) {
            case 'mainMenuMusic':
                this.currentMusic = this.scene.sound.add('mainMenuMusic', { loop: true, volume: 2 });   // Load the main menu music
                if (musicManager.shouldPlayMusic) this.currentMusic.play(); break;                      // IF MUSIC SHOULD PLAY, PLAY IT
            case 'level1Music':
                this.currentMusic = this.scene.sound.add('level1Music', { loop: true, volume: 0.5 });   // Load level 1 music
                if (musicManager.shouldPlayMusic) this.currentMusic.play(); break;                      // IF MUSIC SHOULD PLAY, PLAY IT
            default:
